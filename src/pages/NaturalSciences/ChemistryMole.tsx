...
const IntroScreen = ({ onStart }: { onStart: any }) => (
  <div className="flex flex-col items-center justify-center min-h-[50vh] text-center p-6 bg-white rounded-xl shadow-lg animate-fade-in">
...
export default function ChemistryMole() {
  ...
  const handleStart = () => { setMode('learn'); setLearnStep(0); };
  const handleNextLearn = () => {
    if (learnStep < concepts.length - 1) { setLearnStep(prev => prev + 1); } 
    else { setMode('quiz'); setQuizStep(0); setScore(0); setShowFeedback(false); }
  };
  ...
  const handleNextQuestion = () => {
    if (quizStep < questions.length - 1) { setQuizStep(prev => prev + 1); setShowFeedback(false); } 
    else { setMode('summary'); }
  };
  const handleRestart = () => { setMode('intro'); setScore(0); setQuizStep(0); setLearnStep(0); setShowFeedback(false); };
    ...
    <div className="min-h-screen bg-slate-100 font-sans text-gray-800 py-8 px-4">
      ...
      <div className="max-w-2xl mx-auto mb-6 flex justify-between items-center">
        <div className="flex items-center gap-2">
           <button onClick={() => navigate('/')} className="p-2 hover:bg-slate-200 rounded-full transition-colors text-slate-600" title="回首頁">
             <Home size={24} />
           </button>
      ...
      {mode !== 'intro' && mode !== 'summary' && (
        <div className="max-w-2xl mx-auto mb-8 bg-gray-300 rounded-full h-2.5 overflow-hidden">
          <div className="bg-blue-600 h-2.5 rounded-full transition-all duration-500 ease-out" style={{ width: `${getProgress()}%` }}></div>
        </div>
      )}

      <main className="max-w-2xl mx-auto">
        {mode === 'intro' && <IntroScreen onStart={handleStart} />}
        {mode === 'learn' && <LearningSlide data={concepts[learnStep]} onNext={handleNextLearn} isLast={learnStep === concepts.length - 1} />}
        {mode === 'quiz' && <QuizCard questionData={questions[quizStep]} onAnswer={handleAnswer} feedback={feedback} showFeedback={showFeedback} onNextQuestion={handleNextQuestion} />}
        {mode === 'summary' && <SummaryScreen score={score} total={questions.length} onRestart={handleRestart} />}
      </main>
    </div>
  );
}